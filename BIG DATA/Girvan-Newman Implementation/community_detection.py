        ...
        if my_option_list[0] == 'L':
            ...
            if len(my_option_list) > 2:
                ...
                if NUM_LINKS > MAX_NUM_LINKS or NUM_LINKS < 1:
                    ...
                    G,node_names_list= STUDENT_4691_read_graph_from_csv(NUM_LINKS)
            ...
            if my_option_list[0] == 'RE':                    # 2.1: ADD RANDOM EDGES TO NODES...
                ...
                if len(my_option_list) > 3:
                    ...
                    if NUM_RANDOM_EDGES-1 not in range(5):
                        ...
                        STUDENT_4691_add_random_edges_to_graph(G,node_names_list,NUM_RANDOM_EDGES,EDGE_ADDITION_PROBABILITY)

            elif my_option_list[0] == 'H':                  #2.2: ADD HAMILTON CYCLE...

                    STUDENT_4691_add_hamilton_cycle_to_graph(G,node_names_list)
                ...
                if NUM_OPTIONS > 4:
                    ...
                    if alg_choice == 'N' and graph_layout in ['spring','circular','random','shell']:
                        community_tuples=STUDENT_4691_use_nx_girvan_newman_for_communities(G)

                    elif alg_choice == 'O'and graph_layout in ['spring','circular','random','shell']:
                        community_tuples=STUDENT_4691_one_shot_girvan_newman_for_communities(G,perc)
                ...
                if NUM_OPTIONS > 5:
                    ...
                    if number_of_divisions < NUM_COMPONENTS:
                        ...
                        hierarchy_of_community_tuples,community_tuples,graph_layout,node_positions= STUDENT_4691_divisive_community_detection(G,number_of_divisions,graph_layout,[],perc,algo)

            elif my_option_list[0] == 'M':      # 2.6: DETERMINE PARTITION OF MIN-MODULARITY, FOR A GIVEN BINARY HIERARCHY OF COMMUNITY PARTITIONS
                community_tuples= STUDENT_4691_determine_opt_community_structure(G,hierarchy_of_community_tuples,graph_layout,node_positions)
                ...
                if(len(community_tuples)==0):
                    ...
                    if graph_layout not in ['spring','random','circular','shell']:
                        ...
                        STUDENT_4691_visualize_communities(G,community_tuples,graph_layout,node_positions)
...
########################################################################################
def STUDENT_4691_read_graph_from_csv(NUM_LINKS):
...
######################################################################################################################
def STUDENT_4691_one_shot_girvan_newman_for_communities(G,percentage):
...
######################################################################################################################
def STUDENT_4691_use_nx_girvan_newman_for_communities(G):
    ...
    return community_tuples
######################################################################################################################
def STUDENT_4691_divisive_community_detection(G,number_of_divisions,graph_layout,node_positions,percentage,method):
    ...
    if(method==1):
        while(len(community_tuples)<numDiv):
            ...
            new_communities=STUDENT_4691_use_nx_girvan_newman_for_communities(maxComp)
        ...
        while(len(community_tuples)<numDiv):
            sortListOfLists(community_tuples)
            maxComp= G.subgraph(community_tuples[0]).copy() #take the graph of the largest community
            new_communities=STUDENT_4691_one_shot_girvan_newman_for_communities(maxComp,percentage)
            community_tuples.append(new_communities[0]) #add the new communities to the tuples
            community_tuples.append(new_communities[1])
            currentTriplet=(community_tuples[0],new_communities[0],new_communities[1])
            community_tuples.pop(0) #remove the old community from the tuples
...
######################################################################################################################
def STUDENT_4691_determine_opt_community_structure(G,hierarchy_of_community_tuples,graph_layout,node_positions):
    ...
    plt.show()
    STUDENT_4691_visualize_communities(G,final_tuples_to_print,graph_layout,node_positions)
    return community_tuples

######################################################################################################################
def STUDENT_4691_add_hamilton_cycle_to_graph(G,node_names_list):
    length_G=len(G)
...
######################################################################################################################
def STUDENT_4691_add_random_edges_to_graph(G,node_names_list,NUM_RANDOM_EDGES,EDGE_ADDITION_PROBABILITY):
    length_G=len(G)
    length_N=len(node_names_list)
...
######################################################################################################################
def STUDENT_4691_visualize_communities(G,community_tuples,graph_layout,node_positions):    
...
########################################################################################
########################### STUDENT_4691 ROUTINES LIBRARY ENDS ###########################
########################################################################################
